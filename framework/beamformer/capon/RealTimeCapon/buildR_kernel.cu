...
{ 
   ...
   uint n_threads_per_block = BUILD_R_NUMBER_OF_THREADS;

   dim3 blockR;
   dim3 gridR;
   size_t shared_mem_size;

   while (n_threads_per_block > 1 && n_threads_per_block >= L) { 

      blockR.x = L;
      blockR.y = n_threads_per_block/L; // TODO: Maybe blockR.x should be rounded up to N*warp_size? Then we need to include somthing like if (blockIdx.x < L) in our kernel
      gridR = getGrid(Nx, Ny, blockR.y, Yavg);

      // Calculate shared memory usage
      shared_mem_size = blockR.y*M;   // add shared memory for x
